...
def get_exact_user_cart(user_id):
    ...
    user_cart = sql.execute('SELECT products.pr_name, user_cart.quantity, user_cart.total_for_price FROM products '
                            'INNER JOIN user_cart ON products.pr_id=user_cart.user_product WHERE user_cart.user_id=?;',
                            (user_id,)).fetchall()
    print(user_cart)
